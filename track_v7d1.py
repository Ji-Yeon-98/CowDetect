                ...
                if len(outputs[i]) > 0:
                    ...
                    if frame_idx % 180 == 0:
                        ...
                        for z in activity_volume.keys():
                            if z in activity_dic.keys(): # activity_dic에 activity_volume의 id 값이 있으면
                                activity_dic[z].append(activity_volume[z]) # activity_volume의 값을 activity_dic에 추가
                            else: # activity_volume에 새로운 id가 추가되면
                                try: # 기존의 배열이 있을 때
                                    activity_dic[z] = [0 for z in range(len(list(activity_dic.values())[0]) - 1)] # 이전 시간의 활동량을 0으로 배열 생성
                                    activity_dic[z].append(activity_volume[z]) # 현재 활동량을 마지막에 추가
                                except: # 기존 배열이 없고 처음일 때
                                    activity_dic[z] = [activity_volume[z]] # 현재 활동량을 가진 배열을  

                            tmp += activity_volume[z] # 각 소의 활동량을 더함
                            activity_volume[z] = 0 # activity_volume의 활동량 값 0으로 초기화
